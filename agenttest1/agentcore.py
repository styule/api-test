...
class Agent:
    def __init__(self):
        # you could allow custom system prompt in future
        self.client = client
        self.functions = functions

    def run(self, prompt: str) -> str:
        # Minimal agent: single-turn message
        messages = [
            {
                "role": "system",
                "content": (
                    "You are an assistant that can use tools. "
                    "When calling calculator function, pass only mathematical "
                    "expression (e.g., '2 + 2 * 3'), with no extra text."
                ),
            },
        ...
        if msg.function_call:
            ...
            # append function output and run again
